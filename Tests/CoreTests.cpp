...
void testSynthPrepareBandsAndVoices()
{
    ...
    synth.setFrame(makeGradientFrame());
...
void testSynthRenderDeterministicBoundedAndNoAllocations()
{
    ...
    const LumaFrame frame = makeGradientFrame();
    a.setFrame(frame);
    b.setFrame(frame);
...
void testOneShotVoiceReleaseAndStop()
{
    ...
    releaseSynth.setFrame(makeGradientFrame());
...
{
    AdditiveSynth normal;
    AdditiveSynth transposed;
    SynthConfig config;
    config.sampleRate = 44100.0;
    config.bandCount = BandCount::Bands64;
    config.attackSeconds = 0.001f;
